...
FastPathProcessor::FastPathProcessor(int fp_id,
                                    ...
                                    StatisticsAggregator* stat_engine)
    : fp_id_(fp_id),
      input_queue_(10000),
      conn_tracker_(fp_id),
      rule_manager_(rule_manager),
      output_callback_(std::move(output_callback)),
      stat_engine_(stat_engine) {
}
...
PacketAction FastPathProcessor::processPacket(PacketJob& job) {
    // 1. Track every single packet for global throughput
    if (stat_engine_) {
        stat_engine_->recordWorkerMetric(fp_id_, 1, job.payload_length, input_queue_.size());
    }
    ...
    // 2. Track Brand New Connections
    if (stat_engine_ && (conn->packets_in + conn->packets_out) == 1) { 
        stat_engine_->recordNewConnection();
        
        // EDGE CASE: If the ConnectionTracker instantly classified it based on port (e.g., Port 443 = HTTPS)
        if (conn->state == ConnectionState::CLASSIFIED) {
            struct in_addr src_addr, dst_addr;
            src_addr.s_addr = job.tuple.src_ip;
            dst_addr.s_addr = job.tuple.dst_ip;
            stat_engine_->recordClassification(conn->sni, appTypeToString(conn->app_type), inet_ntoa(src_addr), inet_ntoa(dst_addr));
        }
    }
    ...
    // Remember what the state was before we inspect the payload
    ...
    // If it JUST became classified during this specific packet's payload inspection!
    if (stat_engine_ && !was_classified && conn->state == ConnectionState::CLASSIFIED) {
        struct in_addr src_addr, dst_addr;
        src_addr.s_addr = job.tuple.src_ip;
        dst_addr.s_addr = job.tuple.dst_ip;
        stat_engine_->recordClassification(
            conn->sni, 
    ...
    // 4. Record ongoing bandwidth for apps that are already classified
    if (stat_engine_ && conn->state == ConnectionState::CLASSIFIED && job.payload_length > 0) {
        stat_engine_->recordAppTraffic(
            appTypeToString(conn->app_type), 
            conn->sni, 
            job.payload_length
        );
...
bool FastPathProcessor::tryExtractSNI(const PacketJob& job, Connection* conn) {
    ...
    if (sni) {
        ...
        // --- ADD THESE LINES TO FEED THE AGGREGATOR ---
...
FPManager::FPManager(int num_fps,
    ...
    for (int i = 0; i < num_fps; i++) {
        auto fp = std::make_unique<FastPathProcessor>(i, rule_manager, output_callback, stat_engine);
